...
label neighbor_quest1:
    ...
    menu:
        ...
        Откуда был шум?":
            jump neighbor_quest1_1
        "Не спрашивать. Поздороваться грубо":
            call bitch(2, "fred_hello1") from _call_bitch
            img 1100
            m "Мне не нужны пожелания доброго дня от своих слуг."
            "У меня и без тебя день хороший и всегда будет таким!"
            call get_to_drive_dialogue() from _call_get_to_drive_dialogue
            return

        "Не спрашивать. Просто поздороваться":
            call bitch(-2, "fred_hello2") from _call_bitch_1
            m "Добрый день, Фред."
            call get_to_drive_dialogue() from _call_get_to_drive_dialogue_1
            return

label neighbor_quest1_1:
    m "К делу, Фред."
