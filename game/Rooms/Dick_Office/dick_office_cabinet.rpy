...
label dick_office_cabinet_teleport:
    if EP1==True:
        jump dick_office_cabinet_teleport_EP1
...
label dick_office_cabinet_environment:
    if EP1==True:
        jump dick_office_cabinet_environment_EP1
    if obj_name == "Monica":
...
label dick_office_cabinet_teleport_EP1:
    if obj_name == "Teleport_Secretary":
        if dickOfficeCabinetStage < 3:
            mt "Я не уйду, пока не дождусь помощи от Дика!"
            return
    return
label dick_office_cabinet_environment_EP1:
    if obj_name == "Monica":
        if dickOfficeCabinetStage == 1:
            m "КТО Я????"
            return
