label EP1_jail_day9:
    ...
    call EP1_textonblack(t_("ДЕНЬ 9"))
    img black_screen
    with Dissolve(1)
    music Jail_Clock
...
label EP1_jail_day9_Monica(obj_name, obj_data):
...
label EP1_jail_day9_Bed(obj_name, obj_data):
    if act == "l":
...
label EP1_jail_day9_Cage(obj_name, obj_data):
    if jailDaySceneStage == 4:
        call EP1_jail_day9_4()
        return
    if act == "l":
...
label EP1_jail_day9_Cage_Interact:
    if jailDaySceneStage == 0:
        # приходит
        music Groove2_85
        img 2409
