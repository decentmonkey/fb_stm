label EP1_jail_day7:
    ...
    call EP1_textonblack(t_("ДЕНЬ 7"))
    img black_screen
    with Dissolve(1)
    music Jail_Clock
...
label EP1_jail_day7_Monica(obj_name, obj_data):
...
label EP1_jail_day7_Bed(obj_name, obj_data):
    if act == "l":
...
label EP1_jail_day7_Cage(obj_name, obj_data):
    if act == "l":
        call EP1_jail_cage()
        call EP1_refresh_scene_fade()
        return
...
label EP1_jail_day7_Cage_Interact:
    if jailDaySceneStage == 0:
